...
class B : public WorkerThread, public Observer {
  ...
  private:
    ...
    static EventMapB sEventHandlersList;  // This contains the list of handler
                                          // functions which can be
                                          // initialized at object init section.

  public:
    B(const char *threadName) : WorkerThread::WorkerThread(threadName) { Init(); }
            ...
            // associated handlers add here.
    {
        sEventHandlersList.insert(
            {ClassBEventIds::TASKB_MESSAGE_PRINT_LOG,
             std::bind(&B::HandlePrintLogMessage, this, std::placeholders::_1)});
        sEventHandlersList.insert({ClassBEventIds::TASKB_MESSAGE_SEND_TO_MQTT,
                                   std::bind(&B::HandleReadCANBusMessage, this)});
    }
    ...
    ProcessUserEvent(const ThreadMsg *incoming) override {
        ...
        const DataMsg<UserDataB> *test =
            static_cast<const DataMsg<UserDataB> *>(userData);

        // std::cout << test->getPayload().msg.c_str() << std::endl
        //           << static_cast<int>(test->getPayload().evId) << std::endl
        //           << "on :" << this->getThreadName() << std::endl;

        ASSERT_TRUE((test->getPayload().evId >= ClassBEventIds::TASKB_MESSAGE_MIN) &&
                    (test->getPayload().evId < ClassBEventIds::TASKB_MESSAGE_MAX));
    ...
    virtual void
    update(ObseverMsg *data) override {
        ...
        std::cout << this->getThreadName() << ":"
                  ...
                  << std::endl
                  << "Data val: " << test->getPayload() << std::endl
                  << std::endl;
